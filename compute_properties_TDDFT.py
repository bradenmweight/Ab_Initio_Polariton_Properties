...
def getGlobals():
    ...
    #eta_list = [ 0.0, 0.02, 0.05, 0.06, 0.08, 0.10 ]  # np.array([ 0.0, 0.01, 0.05, 0.1, 0.2 ])  #np.arange( 0.0, 0.2+0.05, 0.05 )
                    ...
                    for y in range(Nxyz[1]):
                        ...
                        for z in range(Nxyz[2]):
                            ...
                            if ( len(outArray) % 6 == 0 or z == Nxyz[2]-1 ):
                                ...
                                f.write( " ".join(map( "{:1.7f}".format, outArray )) + "\n" )
                                outArray = []
                f.close()
...
def compute_Transition_density_matrix_1r( Upol, TDM_matter ):
    ...
    for etaIND in range( len(eta_list) ):
        for p in range( NPolCompute ):
            ...
            plt.plot( np.abs( (TDM_pol[etaIND,p])[np.diag_indices(len(TDM_pol[0,0]))] ) , "-", c="black" )
            plt.xlabel( r"R ($X_h = X_e$) Electron ($\AA$)" ,fontsize=15)
            plt.ylabel( "|TDM|" ,fontsize=15)
            plt.title( r"TDM $P_0$ $\rightarrow$ $P_{}$,   $A_0$ = {}  a.u.".format(p,eta_list[etaIND]) ,fontsize=15)
...
def plotSpectra(Epol,eff_osc_str):
    ...
    plt.imshow( Spec, origin='lower', interpolation='gaussian', extent=(np.amin(energy), np.amax(energy), np.amin(eta_list), np.amax(eta_list)), cmap=cmap, aspect='auto', norm=mpl.colors.Normalize(vmin=0, vmax=1) )
    ...
    #plt.colorbar()
    plt.colorbar(mpl.cm.ScalarMappable(norm=mpl.colors.Normalize(vmin=0, vmax=35), cmap=cmap))
...
def main():
    ...
    #TD_matter = smooth_TD( TDM_matter ) # BE careful with this. Might lead to unexpected results.
